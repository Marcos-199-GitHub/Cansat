...
class Magnetometer:
    def getNorth(mx,my,mz):
        ...
        return angulo
class Accelerometer:
    def getBias(a:np.ndarray):
        #todo
        pass
    def calibrate(a1:np.ndarray,a2:np.ndarray,a3:np.ndarray,a4:np.ndarray,a5:np.ndarray,a6:np.ndarray,):
        #P son las posiciones ideales del acelerometro, que contienen la aceleracion gravitacional a diferentes orientaciones
        """
        p1 = [g,0,0,1]T
        p2 = [-g,0,0,1]T
        p3 = [0,g,0,1]T
        ...
        """
        #Se define una matrix p que es p=[p1,p2,p3,p4,p5,p6]
        p1 = np.array([[G],[0],[0],[1]])
        p2 = np.array([[-G],[0],[0],[1]])
        p3 = np.array([[0],[G],[0],[1]])
        p4 = np.array([[0],[-G],[0],[1]])
        ...
        # Se define una matriz U que es U=[a1,a2,a3,a4,a5,a6]
        U = np.array([a1,a2,a3,a4,a5,a6])
        # Se obtiene la matrix W = U.PT.(P.PT)^-1
...
class Motion:
    ...
    def getYawPitchRoll(a:np.ndarray,rotMatrix:np.ndarray,G:np.ndarray):
        M_PI = 3.14159268
        pitch = 180 * atan (a[0][0]/sqrt(a[0][1]*a[0][1] + a[0][2]*a[0][2]))/M_PI;
        roll = 180 * atan (a[0][1]/sqrt(a[0][0]*a[0][0] + a[0][2]*a[0][2]))/M_PI;
