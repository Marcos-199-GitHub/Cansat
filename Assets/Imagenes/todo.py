...
try:
    gps_adc.readArdu(0x10)
except:
    print("Atmega 328 can't be initialized")
...
btnC.pull = Pull.UP
#LED
eitiLedA = DigitalInOut(board.D4)
...
#XBEE
xbee = serial.Serial('/dev/serial0',baudrate=38400,timeout = 1.0) #1 segundo de timeout, si no, ante cualquier error se congela todo el programa
...
def gps_update(): #Con atmega
    ...
    if (_GPS_TIMEOUT >= MAX_GPS_TIMEOUT_EXCEED):
        return gps_dict
    a = time.time()
    xd = gps_adc.readArdu(0x03) #0x03 es el comando que devuelve una linea del GPS
    xd = str(xd,"ascii","ignore")
    (gps_raw_data, gps_parsed_data) = xd,NMEAReader.parse(xd)
    #print (f"\n{Fore.RED}GPS: {xd}, Parsed: {gps_parsed_data}{Style.RESET_ALL}")
    ...
    if (diff >= 1):    
        _GPS_TIMEOUT+=1
        print(f"Max GPS tiemout exceeded {_GPS_TIMEOUT} times")
    if (gps_raw_data.find("GPRMC") > 0):
        print(".")
        gps_dict = gps_parsed_data.__dict__ 
        print(f"{Fore.CYAN}::{gps_dict}{Style.RESET_ALL}")
    elif (gps_raw_data.find("GPVTG") > 0):
        print(".")
        gps_dict = gps_parsed_data.__dict__
        print(f"{Fore.CYAN}::{gps_dict}{Style.RESET_ALL}")
    elif (gps_raw_data.find("GPGSA") > 0):
        print(".")
        gps_dict = gps_parsed_data.__dict__
        print(f"{Fore.CYAN}::{gps_dict}{Style.RESET_ALL}")
    elif (gps_raw_data.find("GPGGA") > 0):
        print(".")
        gps_dict = gps_parsed_data.__dict__
        print(f"{Fore.CYAN}::{gps_dict}{Style.RESET_ALL}")
...
start = time.time()
last_photo = 0 #Guarda el tiempo de la ultima foto
n=0
total_kb=0
...
diccionario = {
    ...
    "Bl":"0", #Nivel de bateria en porcentaje
    }
...
def main():
    ...
    print("Main")
    f.write(f"\n\n{datetime.datetime.fromtimestamp(datetime.datetime.now().timestamp() - 0*3600).strftime('%c')}\n")
    f.close()
    f = open("data.log","a")
    extT = 0
    relH = 0
    ...
    while True:
        ...
        try:
            ...
            pass
        try:
            xs=str(gps_adc.readArdu(0x01),'ascii','ignore')
            print(xs)
            diccionario["Bl"] = f"{float(xs)}"
        except:
            print("WTF")
            pass
        ...
        if FORMATO == 0:
            ...
            ut+= f"{int(diccionario['Im'])},{diccionario['Bl']}"
            utf = "{" + ut + "}\r\n"
            print (ut)
        ...
        try:
            ...
            print("Can't send data over XBEE")
        e = time.time()
        d = e-s
#        try:
#            s = e
...
#                c+=58
#        except:
#            print("Can't send data over RFM69")
#        e = time.time()
#        d = e-s
...
#
#        except:
#            print("Can't send image over RFM69")
#
#            e = time.time()
        ...
        try:
            if (len(foto)>0):
                z = open("photo_compressed.jpg","rb").read()
...
#                    q = xbee.write(b)
                ...
                q = xbee.write(z)
                e=time.time()
            print(f"{Fore.RED}{(e-s):.3f} s en enviar {Back.GREEN}imagen por XBEE{Style.RESET_ALL}")

        except:
            print("Can't send image over XBEE")
